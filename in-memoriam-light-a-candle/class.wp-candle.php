		...
		if ( strlen( $pre_render ) >= 1 ) {
            ...
            <div class="single-candle candle-<?php echo $this->ID; ?>">
                ...
                <blockquote>
					...
					<?php
					$img_url = get_the_post_thumbnail_url($this->ID);
					$width   = get_option( 'candle_images_width', 200 );
					$height  = get_option( 'candle_images_height', 200 );

					printf( '<img src="%1$s" width="%2$s" height="%3$s">', $img_url, $width, $height );
